...
use super::errors::{
    BStringError, ElementError, FromVariantError, IntoSafeArrElemError, IntoSafeArrayError,
    IntoVariantError, SafeArrayError,
};
...
impl BStringExt for U16String {
    ...
    fn allocate_managed_bstr(&mut self) -> Result<DroppableBString, BStringError> {
        Ok(DroppableBString {
            inner: Some(self.allocate_bstr()?.cast()),
        })
    }

    fn consume_to_managed_bstr(self) -> Result<DroppableBString, BStringError> {
        Ok(DroppableBString {
            inner: Some(self.consume_to_bstr()?.cast()),
        })
    }
...
impl TryConvert<U16String, SafeArrayError> for BSTR {
    ...
    fn try_convert(u: U16String) -> Result<Self, SafeArrayError> {
        match u.clone().allocate_bstr() {
            ...
            Err(bse) => Err(SafeArrayError::from(IntoSafeArrayError::from_element_err(
                IntoSafeArrElemError::from(bse),
                0,
            ))),
        }
    }
